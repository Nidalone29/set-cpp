...
public:
	 ...
	 */
	Set()
		: _head_set(nullptr), _cardinality(0)
	{
#ifndef NDEBUG
	...
	bool add(const value_type& toadd)
	{
		...
		if (this->is_empty()) {
			...
			_cardinality++;
#ifndef NDEBUG
			std::cout << "add(const value_type&)" << " added first value "
				<< toadd << std::endl;
#endif
			return true;
		}
		...
		while (current != nullptr) {
			...
			if (current->next == nullptr) {
				...
				_cardinality++;
#ifndef NDEBUG
				std::cout << "add(const value_type&)" << " added value "
					<< toadd << std::endl;
#endif
				return true;
			}
	 ...
	 */
	bool remove(const value_type& toremove)
	{
		node* current = _head_set;
...
#ifndef NDEBUG
					std::cout << "remove(const value_type&)"
						<< " removed value in the middle " << toremove
						<< std::endl;
#endif
					return;
				}
...
#ifndef NDEBUG
		std::cout << "remove(const value_type&) " << " value not found "
			<< toremove << std::endl;
#endif
	}
	...
	void clear()
	{
		...
		_head_set = nullptr;
#ifndef NDEBUG
		std::cout << "clear()" << " set got cleared " << std::endl;
#endif
	}
...
public:
	...
	bool operator==(const Set<T, Eql>& other)
	{
		...
		for (begin_other = other.begin(), end_other = other.end();
			...
			for (begin_a = this->begin(), end_a = this->begin(); begin_a != end_a; ++begin_a) {
	...
	friend Set<T, Eql> operator-(const Set<T, Eql>& a, const Set<T, Eql>& b)
	{
		...
		try {
			...
			for (begin_a = a.begin(), end_a = a.end(); begin_a != end_a;
				++begin_a) {
				// questa var found è una ottimizzazione per non far continuare 
				// il controllo dopo aver trovato un elemnto comune nel set b
				bool found = false;
				for (begin_b = b.begin(), end_b = b.end(); (begin_b != end_b) &&
					!found; ++begin_b) {
