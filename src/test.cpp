...
void test_accesso_indice()
{
	...
	for (int i = 0; i < set_of_integers.size(); ++i) {
		std::cout << "elemendo a indice " << i << " " << set_of_integers[i] << std::endl;
	}

	std::string elementi_test_str[5] = {
		"ciao",
		"mi",
		"chiamo",
		"giovanni",
	...
	Set<std::string, string_equal> set_of_strings(elementi_test_str, elementi_test_str + 5);
	for (int i = 0; i < set_of_strings.size(); ++i) {
		std::cout << "elemendo a indice " << i << " " << set_of_strings[i] << std::endl;
	}
	...
	Set<Point3D, point_equal> set_of_points(elementi_test_p3d, elementi_test_p3d + 5);
	for (int i = 0; i < set_of_points.size(); ++i) {
		std::cout << "elemendo a indice " << i << " " << set_of_points[i] << std::endl;
	}
}

/**
 * @brief vengono testati tutti gli operatori globali (<< == + -) con un set di interi
 *
 */
void test_operatori_globali_int()
{
	...
	if (set_of_integers == set_of_integers_eql) {
		std::cout << "i due set sono uguali (set_of_integers == set_of_integers_eql)"
			<< std::endl;
	}
	set_of_integers_eql.add(69);
	if (!(set_of_integers == set_of_integers_eql)) {
		std::cout << "i due set non sono uguali (set_of_integers != set_of_integers_eql)"
			<< std::endl;
	}
...
/**
 * @brief vengono testati tutti gli operatori globali (<< == + -) con un set di stringhe
 *
 */
void test_operatori_globali_string()
{
	// set di stringhe
	std::string elementi_test[5] = {
		"ciao",
		"mi",
		"chiamo",
		"giovanni",
	...
	std::string elementi_test_unordered[5] = {
		"wow",
		"ciao",
		"giovanni",
		"mi",
	...
	Set<std::string, string_equal> set_of_strings(elementi_test, elementi_test + 5);
	Set<std::string, string_equal> set_of_strings_eql(elementi_test, elementi_test + 5);
	Set<std::string, string_equal> set_of_strings_2(elementi_test_2, elementi_test_2 + 5);
	...
	// operator + union
	Set<std::string, string_equal> united_string_set = set_of_strings + set_of_strings_2;
	std::cout << "elementi united_string_set: " << united_string_set << std::endl;
	// operator - intersection
	Set<std::string, string_equal> inters_string_set = set_of_strings - set_of_strings_2;
	std::cout << "elementi inters_string_set: " << inters_string_set << std::endl;
}

/**
 * @brief vengono testati tutti gli operatori globali (<< == + -) con un set di dati custom Punto3D
 *
 */
void test_operatori_globali_custom()
{
	...
	Set<Point3D, point_equal> set_of_points_2(elementi_test_2, elementi_test_2 + 5);

	// operator<<
	std::cout << "elementi set_of_points: " << set_of_points << std::endl;
	std::cout << "elementi set_of_points_eql: " << set_of_points_eql
		<< std::endl;
	// operator==
...
void test_filter_out_string()
{
	...
	Set<std::string, string_equal> res = filter_out(set_of_strings, string_evensize_funct);
...
void test_filter_out_custom()
{
	...
	Set<Point3D, point_equal> res = filter_out(set_of_points, point_close_to_center_funct);
...
int main()
{
	...
	std::cout << "Test accesso ai dati tramimte indice (con interi, stringhe e dato custom Punto3D)" << std::endl;
	test_accesso_indice();
